...
public class JdbcUtil {
	...
	/**
	 * 批量执行新增数据
	 * 用于属性名与数据库字段名不一致，且采用下滑线命名规则
	 */
	/*public static boolean formatBatchInsert(JdbcTemplate jdbcTemplate, String table,final List<?> list) {
		if (list == null || list.size() == 0) {
			return false;
		}
		final List<String> col = getTableColName(jdbcTemplate, table);
		String insertSql = getInsertSql(col,table);
		...
		for (String string : col) {
			...
			if(string.contains("_")){
				...
				for(int i=1;i<arr.length;i++){
				    char[] methodName = arr[i].toCharArray();  
				    methodName[0] = toUpperCase(methodName[0]);  
				    arr[i] = String.valueOf(methodName);
				    co+=arr[i];
				}
				string = co;
			}
