function init(){
	...
	$(".product-del").click(function(){
		var that = $(this);
		layer.confirm('您确定要删除当前商品？', {
			btn: ['确定','取消'], //按钮
			title: false,
			icon: 3,
			closeBtn: false
		}, function(i){
			deleteProduct(that.attr("gsId"));
			that.closest(".product-box").remove();
			layer.close(i);
		}, function(i){
			layer.close(i);
		});
