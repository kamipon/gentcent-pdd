...
!function (a) {
	...
	var b, c = function () {
		...
		return {eq:b, eq2:c, peq2:d, ok:e, fail:f, self:i, not:g, and:h, invoke:j, uniqueId:l, rect2bnd:m, invertObject:n, namespaceToCamel:o};
	}(), d = function () {
		var b = function (a) {
			return a[0];
		}, d = function (a) {
		...
		return {head:b, last:d, initial:e, tail:f, prev:r, next:q, find:g, contains:j, all:h, sum:k, from:l, isEmpty:m, clusterBy:n, compact:o, unique:p};
	}(), e = "function" == typeof define && define.amd, f = function (b) {
	...
	var m = {isMac:navigator.appVersion.indexOf("Mac") > -1, isMSIE:h, isEdge:j, isFF:!j && /firefox/i.test(g), isPhantom:/PhantomJS/i.test(g), isWebkit:!j && /webkit/i.test(g), isChrome:!j && /chrome/i.test(g), isSafari:!j && /safari/i.test(g), browserVersion:b, jqueryVersion:parseFloat(a.fn.jquery), isSupportAmd:e, hasCodeMirror:k, isFontInstalled:f, isW3CRangeSupport:!!document.createRange}, n = String.fromCharCode(160), o = "\ufeff", p = function () {
		var b = function (b) {
		...
		}, F = m.isMSIE && m.browserVersion < 11 ? "&nbsp;" : "<br>", G = function (a) {
			...
			var d = [];
			return b = b || c.ok, function e(c) {
				a !== c && b(c) && d.push(c);
				for (var f = 0, g = c.childNodes.length; g > f; f++) {
					e(c.childNodes[f]);
				}
			}(a), d;
		}, R = function (b, c) {
			...
			if (0 === a.offset) {
				...
				aa(a.node) ? (d = a.node.childNodes[a.offset - 1], e = G(d)) : (d = a.node, e = c ? 0 : a.offset - 1);
			}
			...
			if (G(a.node) === a.offset) {
				...
				aa(a.node) ? (d = a.node.childNodes[a.offset], e = 0) : (d = a.node, e = c ? G(a.node) : a.offset + 1);
			}
			return {node:d, offset:e};
		}, da = function (a, b) {
			return a.node === b.node && a.offset === b.offset;
		}, ea = function (a) {
			if (g(a.node) || !aa(a.node) || H(a.node)) {
				return !0;
			}
			...
			var e = a.node.childNodes[a.offset], f = S(a.node.cloneNode(!1), a.node);
			return T(f, P(e)), c || (I(a.node), I(f)), f;
		}, ma = function (a, b, d) {
			var e = L(b.node, c.eq(a));
			return e.length ? 1 === e.length ? la(b, d) : e.reduce(function (a, c) {
				return a === b.node && (a = la(b, d)), la({node:c, offset:a ? p.position(a) : G(c)}, d);
			}) : null;
		}, na = function (a, b) {
			var c, e, f = b ? j : y, g = L(a.node, f), h = d.last(g) || a.node;
			f(h) ? (c = g[g.length - 2], e = h) : (c = h, e = c.parentNode);
			var i = c && ma(c, a, {isSkipPaddingBlankHTML:b, isNotSplitEdgePoint:b});
			return i || e !== a.node || (i = a.node.childNodes[a.offset]), {rightNode:i, container:e};
		}, oa = function (a) {
		...
		return this.memos = {}, this.modules = {}, this.layoutInfo = {}, this.options = e, this.initialize = function () {
			...
			var a = d.head(arguments), e = d.tail(d.from(arguments)), f = this.options.callbacks[c.namespaceToCamel(a, "on")];
			...
			var a = d.head(arguments), b = d.tail(d.from(arguments)), c = a.split("."), e = c.length > 1, f = e && d.head(c), g = e ? d.last(c) : d.head(c), h = this.modules[f || "editor"];
			return !f && this[g] ? this[g].apply(this, b) : h && h[g] && h.shouldInitialize() ? h[g].apply(h, b) : void 0;
		}, this.initialize();
	};
	a.fn.extend({summernote:function () {
		var b = a.type(d.head(arguments)), c = "string" === b, e = "object" === b, f = e ? d.head(arguments) : {};
		...
		if (g.length) {
			...
			if (c) {
				return h.invoke.apply(h, d.from(arguments));
			}
	...
	var r = function (b, c, d, e) {
	...
	}, s = {create:function (b, c) {
		return function () {
			var d = a.isArray(arguments[0]) ? arguments[0] : [], e = "object" == typeof arguments[1] ? arguments[1] : arguments[0];
			return e && e.children && (d = e.children), new r(b, d, e, c);
		};
	}}, t = s.create("<div class=\"note-editor note-frame panel panel-default\"/>"), u = s.create("<div class=\"note-toolbar panel-heading\"/>"), v = s.create("<div class=\"note-editing-area\"/>"), w = s.create("<textarea class=\"note-codable\"/>"), x = s.create("<div class=\"note-editable panel-body\" contentEditable=\"true\"/>"), y = s.create(["<div class=\"note-statusbar\">", "  <div class=\"note-resizebar\">", "    <div class=\"note-icon-bar\"/>", "    <div class=\"note-icon-bar\"/>", "    <div class=\"note-icon-bar\"/>", "  </div>", "</div>"].join("")), z = s.create("<div class=\"note-editor\"/>"), A = s.create("<div class=\"note-editable\" contentEditable=\"true\"/>"), B = s.create("<div class=\"note-btn-group btn-group\">"), C = s.create("<button type=\"button\" class=\"note-btn btn btn-default btn-sm\">", function (a, b) {
		...
		a.html(c.join("")), a.find(".note-color-btn").tooltip({container:"body", trigger:"hover", placement:"bottom"});
	...
	}, J = {editor:t, toolbar:u, editingArea:v, codable:w, editable:x, statusbar:y, airEditor:z, airEditable:A, buttonGroup:B, button:C, dropdown:D, dropdownCheck:E, palette:F, dialog:G, popover:H, icon:I, toggleBtn:function (a, b) {
	...
	var K, L = function () {
		...
		var b = function (a, b) {
			...
			if (0 !== c && p.isText(h[c - 1])) {
				...
				for (var l = k.text.replace(/[\r\n]/g, "").length; l > j.nodeValue.length && j.nextSibling; ) {
					l -= j.nodeValue.length, j = j.nextSibling;
				}
				j.nodeValue;
				b && j.nextSibling && p.isText(j.nextSibling) && l === j.nodeValue.length && (l -= j.nodeValue.length, j = j.nextSibling), f = j, c = l;
			}
			return {cont:f, offset:c};
		}, e = function (a) {
			var b = function (a, e) {
				var f, g;
				if (p.isText(a)) {
					var h = p.listPrev(a, c.not(p.isText)), i = d.last(h).previousSibling;
					f = i || a.parentNode, e += d.sum(d.tail(h), p.nodeLength), g = !i;
				} else {
					if (f = a.childNodes[e] || a, p.isText(f)) {
						return b(f, 0);
					}
					e = 0, g = !1;
				}
				return {node:f, collapseToStart:g, offset:e};
			}, e = document.body.createTextRange(), f = b(a.node, a.offset);
			...
			var j = function () {
				...
				var c = e({node:b, offset:g});
				return c.setEndPoint("EndToEnd", e({node:h, offset:i})), c;
			};
			this.getPoints = function () {
				return {sc:b, so:g, ec:h, eo:i};
			}, this.getStartPoint = function () {
				return {node:b, offset:g};
				...
				var a = function (a, b) {
					if (p.isVisiblePoint(a) && !p.isEdgePoint(a) || p.isVisiblePoint(a) && p.isRightEdgePoint(a) && !b || p.isVisiblePoint(a) && p.isLeftEdgePoint(a) && b || p.isVisiblePoint(a) && p.isBlock(a.node) && p.isEmpty(a.node)) {
						return a;
					}
				...
				a = a || c.ok;
				var e = b && b.includeAncestor, f = b && b.fullyContains, g = this.getStartPoint(), h = this.getEndPoint(), i = [], j = [];
				...
				var c = p.ancestor(b, a), d = p.ancestor(h, a);
				if (!c && !d) {
					return new f(b, g, h, i);
				}
				...
				var b = this.splitText(), c = b.nodes(null, {fullyContains:!0}), e = p.prevPointUntil(b.getStartPoint(), function (a) {
			...
			this.isOnEditable = k(p.isEditable), this.isOnList = k(p.isList), this.isOnAnchor = k(p.isAnchor), this.isOnCell = k(p.isCell), this.isLeftEdgeOf = function (a) {
				if (!p.isLeftEdgePoint(this.getStartPoint())) {
					return !1;
				}
				...
				if (p.isBodyContainer(b) && p.isEmpty(b)) {
					return b.innerHTML = p.emptyPara, new f(b.firstChild, 0, b.firstChild, 0);
				}
				var a = this.normalize();
				if (p.isParaInline(b) || p.isPara(b)) {
					return a;
				}
				var e;
				if (p.isInline(a.sc)) {
					...
					e = a.sc.childNodes[a.so > 0 ? a.so - 1 : 0];
				}
				...
				var b = this.wrapBodyInlineWithPara().deleteContents(), c = p.splitPoint(b.getStartPoint(), p.isInline(a));
				...
				var c = a("<div></div>").html(b)[0], e = d.from(c.childNodes), f = this.wrapBodyInlineWithPara().deleteContents();
				...
				return {s:{path:p.makeOffsetPath(a, b), offset:g}, e:{path:p.makeOffsetPath(a, h), offset:i}};
			}, this.paraBookmark = function (a) {
				return {s:{path:d.tail(p.makeOffsetPath(d.head(a), b)), offset:g}, e:{path:d.tail(p.makeOffsetPath(d.last(a), h)), offset:i}};
		...
		return {create:function (a, b, c, d) {
			...
			var b = a, c = 0, d = a, e = p.nodeLength(d);
			...
			var c = p.fromOffsetPath(a, b.s.path), d = b.s.offset, e = p.fromOffsetPath(a, b.e.path), g = b.e.offset;
			return new f(c, d, e, g);
		}, createFromParaBookmark:function (a, b) {
			var c = a.s.offset, e = a.e.offset, g = p.fromOffsetPath(d.head(b), a.s.path), h = p.fromOffsetPath(d.last(b), a.e.path);
		...
		var b = [], c = -1, d = a[0], e = function () {
			var b = M.create(d), c = {s:{path:[], offset:0}, e:{path:[], offset:0}};
		...
		this.fromNode = function (a) {
			...
			a.each(b.nodes(p.isPara, {includeAncestor:!0}), function (b, d) {
				a(d).css(c);
			});
			...
			if (b.isCollapsed()) {
				return [b.insertNode(p.create(f))];
			}
			var i = p.makePredByNodeName(f), j = b.nodes(p.isText, {fullyContains:!0}).map(function (a) {
			...
			if (g) {
				...
				return j.map(function (b) {
					var c = p.withClosestSiblings(b, i), e = d.head(c), f = d.tail(c);
			...
			var c = a(p.isElement(b.sc) ? b.sc : b.sc.parentNode), d = this.fromNode(c);
			try {
				d = a.extend(d, {"font-bold":document.queryCommandState("bold") ? "bold" : "normal", "font-italic":document.queryCommandState("italic") ? "italic" : "normal", "font-underline":document.queryCommandState("underline") ? "underline" : "normal", "font-subscript":document.queryCommandState("subscript") ? "subscript" : "normal", "font-superscript":document.queryCommandState("superscript") ? "superscript" : "normal", "font-strikethrough":document.queryCommandState("strikethrough") ? "strikethrough" : "normal"});
			}
			catch (e) {
			}
			if (b.isOnList()) {
				var f = ["circle", "disc", "disc-leading-zero", "square"], g = a.inArray(d["list-style-type"], f) > -1;
			...
			if (h && h.style["line-height"]) {
				d["line-height"] = h.style.lineHeight;
			} else {
		...
		this.insertOrderedList = function (a) {
			...
			var e = this, f = M.create(b).wrapBodyInlineWithPara(), g = f.nodes(p.isPara, {includeAncestor:!0}), h = d.clusterBy(g, c.peq2("parentNode"));
			...
			var g = M.create(f).wrapBodyInlineWithPara(), h = g.nodes(p.isPara, {includeAncestor:!0}), i = g.paraBookmark(h), j = d.clusterBy(h, c.peq2("parentNode"));
			if (d.find(h, p.isPurePara)) {
				...
				var l = g.nodes(p.isList, {includeAncestor:!0}).filter(function (b) {
			...
			var c = d.head(a), e = d.last(a), f = p.isList(c.previousSibling) && c.previousSibling, g = p.isList(e.nextSibling) && e.nextSibling, h = f || p.insertAfter(p.create(b || "UL"), e);
			...
			return a.each(b, function (b, f) {
				var g = d.head(f), h = d.last(f), i = c ? p.lastAncestor(g, p.isList) : g.parentNode, j = i.childNodes.length > 1 ? p.splitTree(i, {node:h.parentNode, offset:p.position(h) + 1}, {isSkipPaddingBlankHTML:!0}) : null, k = p.splitTree(i, {node:g.parentNode, offset:p.position(g)}, {isSkipPaddingBlankHTML:!0});
		...
		this.insertTab = function (a, b) {
			...
			if (f) {
				if (p.isEmpty(f) && p.isLi(f)) {
					return void b.toggleList(f.parentNode.nodeName);
				}
				if (p.isEmpty(f) && p.isPara(f) && p.isBlockquote(f.parentNode)) {
					p.insertAfter(f, f.parentNode), e = f;
				} else {
		...
		this.tab = function (a, b) {
			var c = p.ancestor(a.commonAncestor(), p.isCell), e = p.ancestor(c, p.isTable), f = p.listDescendant(e, p.isCell), g = d[b ? "prev" : "next"](f, c);
			g && M.create(g, 0).select();
		}, this.createTable = function (b, c, d) {
			for (var e, f = [], g = 0; b > g; g++) {
				f.push("<td>" + p.blank + "</td>");
			}
			e = f.join("");
	...
	}, T = "bogus", U = function (b) {
		var c = this, e = b.layoutInfo.note, f = b.layoutInfo.editor, g = b.layoutInfo.editable, h = b.options, i = h.langInfo, j = g[0], k = null, l = new P, n = new S, o = new R, q = new Q, r = new O(g);
		this.initialize = function () {
			...
			var c = h.keyMap[m.isMac ? "mac" : "pc"], d = [];
		...
		}, u = ["bold", "italic", "underline", "strikethrough", "superscript", "subscript", "justifyLeft", "justifyCenter", "justifyRight", "justifyFull", "formatBlock", "removeFormat", "backColor", "foreColor", "fontName"], v = 0, w = u.length; w > v; v++) {
			this[u[v]] = function (a) {
				return function (b) {
		...
		this.tab = function () {
			...
			var b = this.createRange(), c = b.insertNode(p.createText(a));
		...
		this.fontSize = function (b) {
			...
			if (c && c.isCollapsed()) {
				var e = l.styleNodes(c), f = d.head(e);
				a(e).css({"font-size":b + "px"}), f && !p.nodeLength(f) && (f.innerHTML = p.ZERO_WIDTH_NBSP_CHAR, M.createFromNodeAfter(f.firstChild).select(), g.data(T, f));
			} else {
				s(), a(l.styleNodes(c)).css({"font-size":b + "px"}), t();
			}
			...
			var a = g.data(T);
			if (a) {
				var b = d.find(d.from(a.childNodes), p.isText), c = b.nodeValue.indexOf(p.ZERO_WIDTH_NBSP_CHAR);
				-1 !== c && b.deleteData(c, 1), p.isEmpty(a) && p.remove(a), g.removeData(T);
			}
		}, this.lineHeight = this.wrapCommand(function (a) {
			l.stylePara(this.createRange(), {lineHeight:a});
			...
			var c = b.url, e = b.text, f = b.isNewWindow, g = b.range || this.createRange(), i = g.toString() !== e;
			...
			if (i) {
				...
				j = l.styleNodes(g, {nodeName:"A", expandClosestSibling:!0, onlyPartialContains:!0});
			}
			a.each(j, function (b, d) {
				a(d).attr("href", c), f ? a(d).attr("target", "_blank") : a(d).removeAttr("target");
			});
			var m = M.createFromNodeBefore(d.head(j)), n = m.getStartPoint(), o = M.createFromNodeAfter(d.last(j)), p = o.getEndPoint();
			M.create(n.node, n.offset, p.node, p.offset).select();
		}), this.getLinkInfo = function () {
			var b = this.createRange().expand(p.isAnchor), c = a(d.head(b.nodes(p.isAnchor)));
			return {range:b, text:b.toString(), isNewWindow:c.length ? "_blank" === c.attr("target") : !1, url:c.length ? c.attr("href") : ""};
		...
		var c = this, e = b.layoutInfo.editable;
		this.events = {"summernote.keydown":function (a, d) {
			...
			this.needKeydownHook() ? (this.$paste = a("<div />").attr("contenteditable", !0).css({position:"absolute", left:-100000, opacity:0}), e.before(this.$paste), this.$paste.on("paste", function (a) {
				b.triggerEvent("paste", a);
			...
			if (p.isImg(c)) {
				for (var d = c.src, e = atob(d.split(",")[1]), f = new Uint8Array(e.length), g = 0; g < e.length; g++) {
					f[g] = e.charCodeAt(g);
				}
		...
		var c = a(document), d = b.layoutInfo.editor, e = b.layoutInfo.editable, f = b.options, g = f.langInfo, h = a(["<div class=\"note-dropzone\">", "  <div class=\"note-dropzone-message\"/>", "</div>"].join("")).prependTo(d);
		this.initialize = function () {
			f.disableDragAndDrop ? c.on("drop", function (a) {
				a.preventDefault();
			}) : this.attachDragAndDropEvent();
		}, this.attachDragAndDropEvent = function () {
			var f = a(), i = h.find(".note-dropzone-message");
	...
	var X = function (a) {
		...
		this.sync = function () {
			...
			var g = p.value(d, e.prettifyHtml) || p.emptyPara, h = c.html() !== g;
		...
		var c = b.layoutInfo.editor, d = b.layoutInfo.toolbar, e = b.layoutInfo.editable, f = b.layoutInfo.codable, g = a(window), h = a("html, body");
		this.toggle = function () {
			var a = function (a) {
			...
			c.toggleClass("fullscreen"), this.isFullscreen() ? (e.data("orgHeight", e.css("height")), g.on("resize", function () {
				a({h:g.height() - d.outerHeight()});
			}).trigger("resize"), h.css("overflow", "hidden")) : (g.off("resize"), a({h:e.data("orgHeight")}), h.css("overflow", "visible")), b.invoke("toolbar.updateFullscreen", this.isFullscreen());
		}, this.isFullscreen = function () {
			return c.hasClass("fullscreen");
		};
	}, _ = function (b) {
		var c = this, d = a(document), e = b.layoutInfo.editingArea, f = b.options;
		this.events = {"summernote.mousedown":function (a, b) {
			...
			var d = p.isImg(c), e = this.$handle.find(".note-control-selection");
			if (b.invoke("imagePopover.update", c), d) {
				var f = a(c), g = f.position(), h = {w:f.outerWidth(!0), h:f.outerHeight(!0)};
				e.css({display:"block", left:g.left, top:g.top, width:h.w, height:h.h}).data("target", f);
				var i = h.w + "x" + h.h;
		...
		var c = this, e = "http://", f = /^(https?:\/\/|ssh:\/\/|ftp:\/\/|file:\/|mailto:[A-Z0-9._%+-]+@)?(www\.)?(.+)$/i;
		this.events = {"summernote.keyup":function (a, b) {
			...
			if (this.lastWordRange) {
				var c = this.lastWordRange.toString(), d = c.match(f);
				if (d && (d[1] || d[2])) {
					var g = d[1] ? c : e + c, h = a("<a />").html(c).attr("href", g)[0];
		...
		this.events = {"summernote.change":function () {
			b.val(a.invoke("code"));
		...
		this.events = {"summernote.init summernote.change":function () {
			c.update();
		...
		var e = this, f = a.summernote.ui, g = b.layoutInfo.toolbar, h = b.options, i = h.langInfo, j = c.invertObject(h.keyMap[m.isMac ? "mac" : "pc"]), k = this.representShortcut = function (a) {
			var b = j[a];
		...
		this.initialize = function () {
			...
			b.memo("button.style", function () {
				return f.buttonGroup([f.button({className:"dropdown-toggle", contents:f.icon(h.icons.magic) + " " + f.icon(h.icons.caret, "span"), tooltip:i.style.style, data:{toggle:"dropdown"}}), f.dropdown({className:"dropdown-style", items:b.options.styleTags, template:function (a) {
					"string" == typeof a && (a = {tag:a, title:a});
					var b = a.tag, c = a.title, d = a.style ? " style=\"" + a.style + "\" " : "", e = a.className ? " className=\"" + a.className + "\"" : "";
					return "<" + b + d + e + ">" + c + "</" + b + ">";
				}, click:b.createInvokeHandler("editor.formatBlock")})]).render();
			}), b.memo("button.bold", function () {
				return f.button({className:"note-btn-bold", contents:f.icon(h.icons.bold), tooltip:i.font.bold + k("bold"), click:b.createInvokeHandler("editor.bold")}).render();
			}), b.memo("button.italic", function () {
				return f.button({className:"note-btn-italic", contents:f.icon(h.icons.italic), tooltip:i.font.italic + k("italic"), click:b.createInvokeHandler("editor.italic")}).render();
			}), b.memo("button.underline", function () {
				return f.button({className:"note-btn-underline", contents:f.icon(h.icons.underline), tooltip:i.font.underline + k("underline"), click:b.createInvokeHandler("editor.underline")}).render();
			}), b.memo("button.clear", function () {
				return f.button({contents:f.icon(h.icons.eraser), tooltip:i.font.clear + k("removeFormat"), click:b.createInvokeHandler("editor.removeFormat")}).render();
			}), b.memo("button.strikethrough", function () {
				return f.button({className:"note-btn-strikethrough", contents:f.icon(h.icons.strikethrough), tooltip:i.font.strikethrough + k("strikethrough"), click:b.createInvokeHandler("editor.strikethrough")}).render();
			}), b.memo("button.superscript", function () {
				return f.button({className:"note-btn-superscript", contents:f.icon(h.icons.superscript), tooltip:i.font.superscript, click:b.createInvokeHandler("editor.superscript")}).render();
			}), b.memo("button.subscript", function () {
				return f.button({className:"note-btn-subscript", contents:f.icon(h.icons.subscript), tooltip:i.font.subscript, click:b.createInvokeHandler("editor.subscript")}).render();
			}), b.memo("button.fontname", function () {
				return f.buttonGroup([f.button({className:"dropdown-toggle", contents:"<span class=\"note-current-fontname\"/> " + f.icon(h.icons.caret, "span"), tooltip:i.font.name, data:{toggle:"dropdown"}}), f.dropdownCheck({className:"dropdown-fontname", checkClassName:h.icons.menuCheck, items:h.fontNames.filter(e.isFontInstalled), template:function (a) {
					return "<span style=\"font-family:" + a + "\">" + a + "</span>";
				}, click:b.createInvokeHandler("editor.fontName")})]).render();
			}), b.memo("button.fontsize", function () {
				return f.buttonGroup([f.button({className:"dropdown-toggle", contents:"<span class=\"note-current-fontsize\"/>" + f.icon(h.icons.caret, "span"), tooltip:i.font.size, data:{toggle:"dropdown"}}), f.dropdownCheck({className:"dropdown-fontsize", checkClassName:h.icons.menuCheck, items:h.fontSizes, click:b.createInvokeHandler("editor.fontSize")})]).render();
			}), b.memo("button.color", function () {
				return f.buttonGroup({className:"note-color", children:[f.button({className:"note-current-color-button", contents:f.icon(h.icons.font + " note-recent-color"), tooltip:i.color.recent, click:function (c) {
				...
				}}), f.button({className:"dropdown-toggle", contents:f.icon(h.icons.caret, "span"), tooltip:i.color.more, data:{toggle:"dropdown"}}), f.dropdown({items:["<li>", "<div class=\"btn-group\">", "  <div class=\"note-palette-title\">" + i.color.background + "</div>", "  <div>", "    <button type=\"button\" class=\"note-color-reset btn btn-default\" data-event=\"backColor\" data-value=\"inherit\">", i.color.transparent, "    </button>", "  </div>", "  <div class=\"note-holder\" data-event=\"backColor\"/>", "</div>", "<div class=\"btn-group\">", "  <div class=\"note-palette-title\">" + i.color.foreground + "</div>", "  <div>", "    <button type=\"button\" class=\"note-color-reset btn btn-default\" data-event=\"removeFormat\" data-value=\"foreColor\">", i.color.resetToDefault, "    </button>", "  </div>", "  <div class=\"note-holder\" data-event=\"foreColor\"/>", "</div>", "</li>"].join(""), callback:function (b) {
					b.find(".note-holder").each(function () {
						var b = a(this);
						b.append(f.palette({colors:h.colors, eventName:b.data("event")}).render());
					});
				}, click:function (c) {
					var d = a(c.target), e = d.data("event"), f = d.data("value");
					if (e && f) {
						var g = "backColor" === e ? "background-color" : "color", h = d.closest(".note-color").find(".note-recent-color"), i = d.closest(".note-color").find(".note-current-color-button");
						h.css(g, f), i.attr("data-" + e, f), b.invoke("editor." + e, f);
					}
				}})]}).render();
			}), b.memo("button.ul", function () {
				return f.button({contents:f.icon(h.icons.unorderedlist), tooltip:i.lists.unordered + k("insertUnorderedList"), click:b.createInvokeHandler("editor.insertUnorderedList")}).render();
			}), b.memo("button.ol", function () {
				return f.button({contents:f.icon(h.icons.orderedlist), tooltip:i.lists.ordered + k("insertOrderedList"), click:b.createInvokeHandler("editor.insertOrderedList")}).render();
			});
			var d = f.button({contents:f.icon(h.icons.alignLeft), tooltip:i.paragraph.left + k("justifyLeft"), click:b.createInvokeHandler("editor.justifyLeft")}), g = f.button({contents:f.icon(h.icons.alignCenter), tooltip:i.paragraph.center + k("justifyCenter"), click:b.createInvokeHandler("editor.justifyCenter")}), j = f.button({contents:f.icon(h.icons.alignRight), tooltip:i.paragraph.right + k("justifyRight"), click:b.createInvokeHandler("editor.justifyRight")}), l = f.button({contents:f.icon(h.icons.alignJustify), tooltip:i.paragraph.justify + k("justifyFull"), click:b.createInvokeHandler("editor.justifyFull")}), m = f.button({contents:f.icon(h.icons.outdent), tooltip:i.paragraph.outdent + k("outdent"), click:b.createInvokeHandler("editor.outdent")}), n = f.button({contents:f.icon(h.icons.indent), tooltip:i.paragraph.indent + k("indent"), click:b.createInvokeHandler("editor.indent")});
			b.memo("button.justifyLeft", c.invoke(d, "render")), b.memo("button.justifyCenter", c.invoke(g, "render")), b.memo("button.justifyRight", c.invoke(j, "render")), b.memo("button.justifyFull", c.invoke(l, "render")), b.memo("button.outdent", c.invoke(m, "render")), b.memo("button.indent", c.invoke(n, "render")), b.memo("button.paragraph", function () {
				return f.buttonGroup([f.button({className:"dropdown-toggle", contents:f.icon(h.icons.alignLeft) + " " + f.icon(h.icons.caret, "span"), tooltip:i.paragraph.paragraph, data:{toggle:"dropdown"}}), f.dropdown([f.buttonGroup({className:"note-align", children:[d, g, j, l]}), f.buttonGroup({className:"note-list", children:[m, n]})])]).render();
			}), b.memo("button.height", function () {
				return f.buttonGroup([f.button({className:"dropdown-toggle", contents:f.icon(h.icons.textHeight) + " " + f.icon(h.icons.caret, "span"), tooltip:i.font.height, data:{toggle:"dropdown"}}), f.dropdownCheck({items:h.lineHeights, checkClassName:h.icons.menuCheck, className:"dropdown-line-height", click:b.createInvokeHandler("editor.lineHeight")})]).render();
			}), b.memo("button.table", function () {
				return f.buttonGroup([f.button({className:"dropdown-toggle", contents:f.icon(h.icons.table) + " " + f.icon(h.icons.caret, "span"), tooltip:i.table.table, data:{toggle:"dropdown"}}), f.dropdown({className:"note-table", items:["<div class=\"note-dimension-picker\">", "  <div class=\"note-dimension-picker-mousecatcher\" data-event=\"insertTable\" data-value=\"1x1\"/>", "  <div class=\"note-dimension-picker-highlighted\"/>", "  <div class=\"note-dimension-picker-unhighlighted\"/>", "</div>", "<div class=\"note-dimension-display\">1 x 1</div>"].join("")})], {callback:function (a) {
					var c = a.find(".note-dimension-picker-mousecatcher");
					c.css({width:h.insertTableMaxSize.col + "em", height:h.insertTableMaxSize.row + "em"}).mousedown(b.createInvokeHandler("editor.insertTable")).on("mousemove", e.tableMoveHandler);
				}}).render();
			}), b.memo("button.link", function () {
				return f.button({contents:f.icon(h.icons.link), tooltip:i.link.link, click:b.createInvokeHandler("linkDialog.show")}).render();
			}), b.memo("button.picture", function () {
				return f.button({contents:f.icon(h.icons.picture), tooltip:i.image.image, click:b.createInvokeHandler("imageDialog.show")}).render();
			}), b.memo("button.video", function () {
				return f.button({contents:f.icon(h.icons.video), tooltip:i.video.video, click:b.createInvokeHandler("videoDialog.show")}).render();
			}), b.memo("button.hr", function () {
				return f.button({contents:f.icon(h.icons.minus), tooltip:i.hr.insert + k("insertHorizontalRule"), click:b.createInvokeHandler("editor.insertHorizontalRule")}).render();
			}), b.memo("button.fullscreen", function () {
				return f.button({className:"btn-fullscreen", contents:f.icon(h.icons.arrowsAlt), tooltip:i.options.fullscreen, click:b.createInvokeHandler("fullscreen.toggle")}).render();
			}), b.memo("button.codeview", function () {
				return f.button({className:"btn-codeview", contents:f.icon(h.icons.code), tooltip:i.options.codeview, click:b.createInvokeHandler("codeview.toggle")}).render();
			}), b.memo("button.redo", function () {
				return f.button({contents:f.icon(h.icons.redo), tooltip:i.history.redo + k("redo"), click:b.createInvokeHandler("editor.redo")}).render();
			}), b.memo("button.undo", function () {
				return f.button({contents:f.icon(h.icons.undo), tooltip:i.history.undo + k("undo"), click:b.createInvokeHandler("editor.undo")}).render();
			}), b.memo("button.help", function () {
				return f.button({contents:f.icon(h.icons.question), tooltip:i.options.help, click:b.createInvokeHandler("helpDialog.show")}).render();
			});
		}, this.addImagePopoverButtons = function () {
			b.memo("button.imageSize100", function () {
				return f.button({contents:"<span class=\"note-fontsize-10\">100%</span>", tooltip:i.image.resizeFull, click:b.createInvokeHandler("editor.resize", "1")}).render();
			}), b.memo("button.imageSize50", function () {
				return f.button({contents:"<span class=\"note-fontsize-10\">50%</span>", tooltip:i.image.resizeHalf, click:b.createInvokeHandler("editor.resize", "0.5")}).render();
			}), b.memo("button.imageSize25", function () {
				return f.button({contents:"<span class=\"note-fontsize-10\">25%</span>", tooltip:i.image.resizeQuarter, click:b.createInvokeHandler("editor.resize", "0.25")}).render();
			}), b.memo("button.floatLeft", function () {
				return f.button({contents:f.icon(h.icons.alignLeft), tooltip:i.image.floatLeft, click:b.createInvokeHandler("editor.floatMe", "left")}).render();
			}), b.memo("button.floatRight", function () {
				return f.button({contents:f.icon(h.icons.alignRight), tooltip:i.image.floatRight, click:b.createInvokeHandler("editor.floatMe", "right")}).render();
			}), b.memo("button.floatNone", function () {
				return f.button({contents:f.icon(h.icons.alignJustify), tooltip:i.image.floatNone, click:b.createInvokeHandler("editor.floatMe", "none")}).render();
			}), b.memo("button.removeMedia", function () {
				return f.button({contents:f.icon(h.icons.trash), tooltip:i.image.remove, click:b.createInvokeHandler("editor.removeMedia")}).render();
			});
		}, this.addLinkPopoverButtons = function () {
			b.memo("button.linkDialogShow", function () {
				return f.button({contents:f.icon(h.icons.link), tooltip:i.link.edit, click:b.createInvokeHandler("linkDialog.show")}).render();
			}), b.memo("button.unlink", function () {
				return f.button({contents:f.icon(h.icons.unlink), tooltip:i.link.unlink, click:b.createInvokeHandler("editor.unlink")}).render();
			});
		}, this.build = function (a, c) {
			for (var d = 0, e = c.length; e > d; d++) {
				for (var g = c[d], h = g[0], i = g[1], j = f.buttonGroup({className:"note-" + h}).render(), k = 0, l = i.length; l > k; k++) {
			...
			if (this.updateBtnStates({".note-btn-bold":function () {
			...
			var c, d = 18, e = a(b.target.parentNode), f = e.next(), g = e.find(".note-dimension-picker-mousecatcher"), i = e.find(".note-dimension-picker-highlighted"), j = e.find(".note-dimension-picker-unhighlighted");
			if (void 0 === b.offsetX) {
				var k = a(b.target).offset();
				c = {x:b.pageX - k.left, y:b.pageY - k.top};
			} else {
				c = {x:b.offsetX, y:b.offsetY};
			}
			var l = {c:Math.ceil(c.x / d) || 1, r:Math.ceil(c.y / d) || 1};
			i.css({width:l.c + "em", height:l.r + "em"}), g.data("value", l.c + "x" + l.r), 3 < l.c && l.c < h.insertTableMaxSize.col && j.css({width:l.c + 1 + "em"}), 3 < l.r && l.r < h.insertTableMaxSize.row && j.css({height:l.r + 1 + "em"}), f.html(l.c + " x " + l.r);
		};
	}, ea = function (b) {
		var c = a.summernote.ui, d = b.layoutInfo.note, e = b.layoutInfo.toolbar, f = b.options;
		this.shouldInitialize = function () {
			return !f.airMode;
		}, this.initialize = function () {
		...
		this.initialize = function () {
			...
			return a.Deferred(function (a) {
				var f = c.$dialog.find(".note-link-text"), g = c.$dialog.find(".note-link-url"), h = c.$dialog.find(".note-link-btn"), i = c.$dialog.find("input[type=checkbox]");
				d.onDialogShown(c.$dialog, function () {
					b.triggerEvent("dialog.shown"), f.val(e.text), f.on("input", function () {
						...
						b.preventDefault(), a.resolve({range:e.range, url:g.val(), text:f.val(), isNewWindow:i.is(":checked")}), c.$dialog.modal("hide");
					});
		...
		var c = this, e = a.summernote.ui, f = b.options;
		this.events = {"summernote.keyup summernote.mouseup summernote.change summernote.scroll":function () {
			c.update();
			...
			this.$popover = e.popover({className:"note-link-popover", callback:function (a) {
			...
			if (!b.invoke("editor.hasFocus")) {
				return void this.hide();
			}
			var c = b.invoke("editor.createRange");
			if (c.isCollapsed() && c.isOnAnchor()) {
				var d = p.ancestor(c.sc, p.isAnchor), e = a(d).attr("href");
		...
		this.initialize = function () {
			...
			if (f.maximumImageFileSize) {
				var h = Math.floor(Math.log(f.maximumImageFileSize) / Math.log(1024)), i = 1 * (f.maximumImageFileSize / Math.pow(1024, h)).toFixed(2) + " " + " KMGTP"[h] + "B";
				c = "<small>" + g.image.maximumFileSize + " : " + i + "</small>";
			}
			var j = "<div class=\"form-group note-group-select-from-files\"><label>" + g.image.selectFromFiles + "</label><input class=\"note-image-input form-control\" type=\"file\" name=\"files\" accept=\"image/*\" multiple=\"multiple\" />" + c + "</div><div class=\"form-group\" style=\"overflow:auto;\"><label>" + g.image.url + "</label><input class=\"note-image-url form-control col-md-12\" type=\"text\" /></div>", k = "<button href=\"#\" class=\"btn btn-primary note-image-btn disabled\" disabled>" + g.image.insert + "</button>";
			...
			return a.Deferred(function (a) {
				var e = c.$dialog.find(".note-image-input"), f = c.$dialog.find(".note-image-url"), g = c.$dialog.find(".note-image-btn");
				d.onDialogShown(c.$dialog, function () {
		...
		var c = a.summernote.ui, e = b.options;
		this.shouldInitialize = function () {
			return !d.isEmpty(e.popover.image);
		}, this.initialize = function () {
			this.$popover = c.popover({className:"note-image-popover"}).render().appendTo("body");
			var a = this.$popover.find(".popover-content");
		...
		var c = this, d = a.summernote.ui, e = b.layoutInfo.editor, f = b.options, g = f.langInfo;
		this.initialize = function () {
			var b = f.dialogsInBody ? a(document.body) : e, c = "<div class=\"form-group row-fluid\"><label>" + g.video.url + " <small class=\"text-muted\">" + g.video.providers + "</small></label><input class=\"note-video-url form-control span12\" type=\"text\" /></div>", h = "<button href=\"#\" class=\"btn btn-primary note-video-btn disabled\" disabled>" + g.video.insert + "</button>";
			...
			var c, d = /^(?:https?:\/\/)?(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))((\w|-){11})(?:\S+)?$/, e = b.match(d), f = /\/\/instagram.com\/p\/(.[a-zA-Z0-9_-]*)/, g = b.match(f), h = /\/\/vine.co\/v\/(.[a-zA-Z0-9]*)/, i = b.match(h), j = /\/\/(player.)?vimeo.com\/([a-z]*\/)*([0-9]{6,11})[?]?.*/, k = b.match(j), l = /.+dailymotion.com\/(video|hub)\/([^_]+)[^#]*(#video=([^_&]+))?/, m = b.match(l), n = /\/\/v\.youku\.com\/v_show\/id_(\w+)=*\.html/, o = b.match(n), p = /^.+.(mp4|m4v)$/, q = b.match(p), r = /^.+.(ogg|ogv)$/, s = b.match(r), t = /^.+.(webm)$/, u = b.match(t);
			if (e && 11 === e[1].length) {
				var v = e[1];
				c = a("<iframe>").attr("frameborder", 0).attr("src", "//www.youtube.com/embed/" + v).attr("width", "640").attr("height", "360");
			} else {
				if (g && g[0].length) {
					c = a("<iframe>").attr("frameborder", 0).attr("src", g[0] + "/embed/").attr("width", "612").attr("height", "710").attr("scrolling", "no").attr("allowtransparency", "true");
				} else {
					if (i && i[0].length) {
						c = a("<iframe>").attr("frameborder", 0).attr("src", i[0] + "/embed/simple").attr("width", "600").attr("height", "600").attr("class", "vine-embed");
					} else {
						if (k && k[3].length) {
							c = a("<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>").attr("frameborder", 0).attr("src", "//player.vimeo.com/video/" + k[3]).attr("width", "640").attr("height", "360");
						} else {
							if (m && m[2].length) {
								c = a("<iframe>").attr("frameborder", 0).attr("src", "//www.dailymotion.com/embed/video/" + m[2]).attr("width", "640").attr("height", "360");
							} else {
								if (o && o[1].length) {
									c = a("<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>").attr("frameborder", 0).attr("height", "498").attr("width", "510").attr("src", "//player.youku.com/embed/" + o[1]);
								} else {
									if (!(q || s || u)) {
										return !1;
									}
		...
		this.createShortCutList = function () {
			...
			var b = f.dialogsInBody ? a(document.body) : e, c = ["<p class=\"text-center\">", "<a href=\"//summernote.org/\" target=\"_blank\">Summernote 0.8.1</a> \xb7 ", "<a href=\"//github.com/summernote/summernote\" target=\"_blank\">Project</a> \xb7 ", "<a href=\"//github.com/summernote/summernote/issues\" target=\"_blank\">Issues</a>", "</p>"].join("");
			this.$dialog = d.dialog({title:g.options.help, fade:f.dialogsFade, body:this.createShortCutList(), footer:c, callback:function (a) {
		...
		var e = this, f = a.summernote.ui, g = b.options, h = 20;
		this.events = {"summernote.keyup summernote.mouseup summernote.scroll":function () {
			e.update();
			...
			this.$popover = f.popover({className:"note-air-popover"}).render().appendTo("body");
			var a = this.$popover.find(".popover-content");
			...
			if (a.range && !a.range.isCollapsed()) {
				...
				if (e) {
					var f = c.rect2bnd(e);
					this.$popover.css({display:"block", left:Math.max(f.left + f.width / 2, 0) - h, top:f.top + f.height});
				}
			} else {
				this.hide();
			}
		...
		var e = this, f = a.summernote.ui, g = 5, h = b.options.hint || [], i = b.options.hintDirection || "bottom", j = a.isArray(h) ? h : [h];
		this.events = {"summernote.keyup":function (a, b) {
			...
			this.lastWordRange = null, this.$popover = f.popover({className:"note-hint-popover", hideArrow:!0, direction:""}).render().appendTo("body"), this.$popover.hide(), this.$content = this.$popover.find(".popover-content"), this.$content.on("click", ".note-hint-item", function () {
			...
			var b = j[a.data("index")], c = a.data("item"), d = b.content ? b.content(c) : c;
			...
			if (d.contains([L.code.ENTER, L.code.UP, L.code.DOWN], a.keyCode)) {
				if (a.keyCode === L.code.ENTER && this.$popover.is(":visible")) {
					return;
				}
			} else {
				var f = b.invoke("editor.createRange").getWordRange(), h = f.toString();
				if (j.length && h) {
					this.$content.empty();
					...
					}), "top" === i ? this.$popover.css({left:k.left, top:k.top - this.$popover.outerHeight() - g}) : this.$popover.css({left:k.left, top:k.top + k.height + g}));
				} else {
					this.hide();
				}
			}
