                    ...
                    <div class="ibox-title">
                        ...
                        <div class="ibox-tools" style="position:relative;top:-30px;">
                       		<c:choose>
	                       		...
	                       		<!--<span class="glyphicon glyphicon-plus"></span>
	                       		<font color=red style="font-size:10px;">（*注意:快到期的商家排前面，已过期的商家整行变红，注意联系商家续费）</font>
	                      ...
	                      <table class="table table-striped table-bordered table-hover dataTables-example" id="editable" style="table-layout:fixed">
	                            <thead class="pn-lthead">
	                                <tr>
	                                    <th>商家名</th>
                                    	<th>联系方式</th>
                                    	<c:if test="${state==true}"><th>账号</th></c:if>
                                    	<th>状态</th>
                                    	<!-- 
	                            ...
	                            <tbody  id="idsVal">
	                            	<c:forEach items="${list}" var="items">
	                            		<c:choose>
		                            		...
		                            		<td>
		                            			<c:choose>
		                            				<c:when test="${items.activity.type=='0'}">
		                            					试用
		                            				</c:when>
		                            				<c:when test="${items.activity.type=='1'}">
		                            					正式
		                            				</c:when>
		                            			</c:choose>
		                            		</td>
...
<!--		                            		<td>-->
<!--		                            		<fmt:formatNumber value="${items.xiaohaoMoney}" pattern="0.00"/>	-->
<!--		                            		</td>-->
<!--		                            		<td>-->
<!--		                            		<fmt:formatNumber value="${items.toufangMoney }" pattern="0.00"/>	-->
<!--		                            		</td>-->
		                            		<!--  
		                            		<td>
		                            			<c:if test="${items.activity.pattern==0}">分享领取</c:if>
		                            			<c:if test="${items.activity.pattern==1}">直接领取</c:if>
		                            			<c:if test="${items.activity.pattern==2}">关注领取</c:if>
		                            			||
		                            			<c:if test="${items.activity.saveType==0}">领到余额</c:if>
		                            			<c:if test="${items.activity.saveType==1}">直接转账</c:if>
		                            		</td>
		                            		-->
<!--		                            		<td id="redpacketNum">-->
<!--		                            			${items.activity.num }-->
...
<!--		                                		|<a href="javascript:void(0)" onclick="setMoney('${items.id }')">充值余额</a>-->
		                                		<!--<a href="javascript:void(0)" onclick="initMoney('${items.id }')">初始化余额</a>|
		                                		-->
		                                		<!-- 
		                                		|
		                                		<c:if test="${state==true}">
		                                		<a href="javascript:void(0)" onclick="setRedPacket('${items.id }','${items.num }','${items.name }')">绑定红包</a>|
		                                		</c:if>
		                                		<c:if test="${state==true}">
		                                		<c:if test="${items.isBoolean==true}">
		                                		<a href="javascript:void(0)" onclick="setBoolean('${items.id }','${items.isBoolean }')">关闭多领取</a>
		                                		</c:if>
		                                		<c:if test="${items.isBoolean==false}">
		                                		<a href="javascript:void(0)" onclick="setBoolean('${items.id }','${items.isBoolean }')">开启多领取</a>
		                                		</c:if>
		                                		</c:if>
		                                		 -->
		                            		</td>
