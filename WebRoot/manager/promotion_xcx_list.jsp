            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
						<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper form-inline" role="grid" style="padding-bottom: 10px">
							<form action="${pageContext.request.contextPath }/promotion/all" method="get">
								<div class="row">
									<div class="col-sm-6">
									...
									<button onclick="bang()" type="button" class="btn btn-primary"  style="margin-left:50%">解绑或绑定小程序</button>
										<button onclick="addhaibao()" type="button" class="btn btn-primary" style="margin-left:<c:if test="${empty isBang }">68%</c:if><c:if test="${!empty isBang }">1%</c:if>">上传海报</button>
										<div id="DataTables_Table_0_length" class="dataTables_filter" style="margin-top: -36px">
											<label>每页
								...
								<font style="color: red">*以下活动及排名与小程序一致</font>
						</div>
	                            ...
	                            <tbody id="user" style="height: 10%">
	                            	<c:forEach items="${list}" var="items" varStatus="status">
	                            		<tr style="height: 60px"> 
		                            		...
		                            		<td>
		                            			<c:if test="${items.isStick==1 }">是</c:if>
		                            			<c:if test="${items.isStick==0 }">否</c:if>
		                            		</td>
		                            		<td>
		                            			${items.num }
		                            		</td>
		                            		...
		                            		<td>
		                            			<a class="pn-opt" href="javascript:void(0);" onclick="del('${items.id}')"><c:if test="${items.isStick==1 }">取消推荐</c:if><c:if test="${items.isStick==0 }">推荐</c:if></a>|
		                                		<a class="J_menuItem"  onclick="update('${items.id}')">更改排名</a>|
		                                		<a class="J_menuItem"  onclick="del1('${items.id}')">关闭</a>
		                            		</td>
	                            		</tr>
	                            	</c:forEach>
	                            </tbody>
		                        ...
		                        <tbody>
		                            <tr>
		                                <td align="center" class="pn-sp">
		                                    <form id="paging" action="promotion/all" method="get">
		...
		function update(id){
			...
			function(inputValue){	
				...
				$.ajax({
						...
						success:function(data){
							if(data.flag){
								swal({title:"更改排名成功！",type:"success"},function(){
									window.location.reload();
								});
							}else{
								layer.msg(data.msg);
      		...
      		function del1(id){
			    $.ajax({
						...
						success:function(data){
							if(data.flag){
								swal({title:"活动已关闭",type:"success"},function(){
									window.location.reload();
								});
							}else{
								layer.msg(data.msg);
