	...
	<script type="text/javascript">
		...
		function ajaxContentList(param) {
			$.ajax({
				async:false,
				cache:false,
				url:"${cp }/wlspecs/ajax/"+param.navId,
				type:"post",
				dataType:"json",
				error:function(data){
					layer.msg("请求失败");
		...
		$("#add_fath_article").click(function(){
		   layer.open({
		      ...
		      shade: 0.8,
		      area: ['1200px', '90%'],
		      content: '${cp }/wlspecs/toadd' //iframe的url
		   });
		 });
	</script>
</head>
												...
												<div class="form-group">
													...
													<div class="col-sm-10">
														<a data-toggle="modal" class="btn btn-primary"
															href="javascript:void(0)" onclick="choose()">选择规格</a>
...
<script type="text/javascript">
	...
	$("#add_son_article").click(function(){
		...
		var nodes = zTree.getSelectedNodes();
		if(nodes == ''){
			layer.msg("请选中需要更改的规格!");
			return false;
		}
		var level = nodes.level; 
		if(level==2){
			layer.msg("规格无法创建超过3级!");
			return false;
		}
		var idVal = nodes[0].id;
		  layer.open({
			  ...
			  shade: 0.8,
			  area: ['1200px', '90%'],
			  content: '${cp }/wlspecs/toadd?parentId='+idVal //iframe的url
		});			
	});
	...
	$("#delete_article").click(function(){
		...
		var nodes = zTree.getSelectedNodes();
		if(nodes == ''){
			layer.msg("请选中需要删除的规格!");
			return false;
		}
		var idVal = nodes[0].id;
		$.ajax({
			type:"post",
			url:"${cp }/wlspecs/"+idVal,
			data:{'_method':'delete'},
			dataType:"json",
			success:function(data){
				if(data.flag){
	...
	$("#add_ter").click(function(){
		...
		var array=[[name,"请填写规格名称!"],[order,"请填写排序!"],[code,"请填写编号!"]];
	...
	function choose(){
	   layer.open({
			...
			shade: 0.8,
			area: ['800px', '90%'],
			content: '${cp }/manager/wl_specs_choose.jsp' //iframe的url
	   });
	}
