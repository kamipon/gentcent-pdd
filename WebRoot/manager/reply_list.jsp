                ...
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
						...
						<div class="ibox-tools">
							...
							<a class="J_menuItem" href="manager/reply_add.jsp">添加</a>
						</div>
                    </div>
	                            ...
	                            <tbody  id="idsVal" class="list">
	                            	<c:forEach items="${list}" var="items" varStatus="s">
	                            		<tr>
		                            		...
		                            		<td>
												<c:if test="${items.replytype == '图文' }">
													<img src="${items.imgUrl}" width="80" height="50">
												</c:if>
												<c:if test="${items.replytype == '文本' }">
													<span style="color: #999999;">回复文本消息无图片</span>
												</c:if>
												<c:if test="${items.replytype == '图片' }">
													<img src="${items.imgUrl}" width="80" height="50">
												</c:if>
		                            		</td>
		                            		<td>
												...
												<c:if test="${items.replytype == '文本' }">
													<span style="color: #999999;">回复文本消息无标题</span>
												</c:if>
												<c:if test="${items.replytype == '图片' }">
													<span style="color: #999999;">回复图片消息无标题</span>
												</c:if>
		                            		</td>
		                            		<td>
		                            			${items.key}
		                            		...
		                            		<td>
												...
												<c:if test="${items.replytype == '图片' }">
													<span style="color: #999999;">回复图片消息无回复内容</span>
												</c:if>
		                            		</td>
		                            		<td>
		                            			<fmt:formatDate value="${items.addTime}"	type="both" />
		                            		</td>
		                            		<td>
		                            			<a class="J_menuItem" href="reply/toUpdate/${items.id}" name="j23">修改</a> |
												<a class="pn-opt"
													onclick="delContent('${items.id}')" href="javascript:;" name="j24">删除</a>
		                            		</td>
	                            		</tr>
	                            	</c:forEach>
	                            </tbody>
		                        ...
		                        <tbody>
		                            <tr>
		                                <td align="center" class="pn-sp">
		                                    <form id="paging" action="${pageContext.request.contextPath }/reply" method="get">
