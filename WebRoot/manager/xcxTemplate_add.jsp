                    ...
                    <div class="ibox-content">
	                    <form method="post" class="form-horizontal" id="modularForm" >
	                        <div class="form-group">
	                            ...
	                            <div class="col-sm-10">
	                                <select  id="template" onchange="change();" class="form-control">
	                                	<c:if test="${empty list}">
	                                		<option value="">暂无模板可用！</option>
	                                	</c:if>
	                                	<c:forEach items="${list}" var="item">
	                                		<option value="${item.template_id }" <c:if test="${item.template_id==template_id }">selected="selected"</c:if>>${item.title}</option>
	                                	</c:forEach>
	                                </select>
	                            </div>
	                        </div>
...
<script >
	function change(){
		var template_id = $.trim($("#template").val())
		location='xcxTemplate/toadd?template_id='+template_id;
	}
	$(function(){
		$("#update_article").click(function(){
			...
			//标题
			var template_id = $.trim($("option:selected").val());
			//标题
			var title = $.trim($("option:selected").text());
			//内容
			var data = '\{';
			var keywords = '\{';
