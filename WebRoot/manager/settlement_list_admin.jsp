                ...
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
						<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper form-inline" role="grid">
							<form action="${pageContext.request.contextPath }/settlement/listAdmin" method="get">
								<div class="row">
									<div class="col-sm-6">
										<div class="dataTables_length" id="DataTables_Table_0_filter">
											...
											<input   value="${type}" name="type" type="hidden">
					                        	...
					                        	<c:if test="${type eq '2'}"><input type="text"  value="${terName}" name="terName" placeholder="根据代理商查询"></c:if>
					                        	<c:if test="${type eq '1'}"><input type="text"  value="${actName}" name="actName" placeholder="根据商家查询"></c:if>
	                      ...
	                      <table class="table table-striped table-bordered table-hover dataTables-example" id="editable" style="table-layout:fixed">
	                            <thead class="pn-lthead">
	                                <tr>
	                                	<c:if test="${type eq '2'}"><th>代理</th></c:if>
	                                	<th>商家</th>
	                                    <th>结算单号</th>
	                                    <th>结算总金额</th>
	                                    <th>手续费</th>
	                                   <c:if test="${type eq '1'}"> <th>实际结算金额</th></c:if>
	                                    <th>添加时间</th>
                                    	<th>操作</th>                                 
	                                </tr>
	                            </thead>
	                            ...
	                            <c:forEach items="${list}" var="items">
	                           				 <c:if test="${type eq '2'}"><th>${items.terpoint}</th></c:if>
		                            		<td>
		                            			${items.activity}
		                            		</td>
		                            		<td>
		                        ...
		                        <tbody>
		                            <tr>
		                                <td align="center" class="pn-sp">
		                                    <form id="paging" action="settlement/listAdmin" method="get">
		                                    <input value="${type}" name="type" type="hidden">
											...
											<input type="hidden"  value="${terName}" name="terName" >
		  ...
		  function count(id){
		    swal({
		        ...
		        closeOnConfirm: false
		    		...
		    		$.ajax({
		    			type:"post",
						url:"settlement/count?id="+id,
						data:{
						},
						dataType:"json",
						success:function(data){
							if(data.flag){
								swal({title:"结算成功！",type:"success"},function(){
