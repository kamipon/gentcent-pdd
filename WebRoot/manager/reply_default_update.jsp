...
<head>
	...
	<link href="${cp }/js/plugins/sumoselect/sumoselect.css" rel="stylesheet">
	...
	<script src="${cp }/js/plugins/sumoselect/jquery.sumoselect.js"></script>
	<script type="text/javascript" src="${cp }/js/layer/layer.js"></script>
	<script type="text/javascript" src="${cp }/js/11cms.js"></script>
	<link type="text/css" rel="stylesheet" href="${cp }/css/statistics.css">
                        ...
                        <form method="post" class="form-horizontal" action="${cp}/reply">
                            <div class="form-group">
								<label class="col-sm-2 control-label">
									<font color=red>*</font>非关键字默认回复
								</label>
								<div class="col-sm-10">
									<input name="replytype" type="hidden" value="${reply.replytype }" id="replytype"/>
									<c:choose>
							...
							<c:if test="${reply.replytype=='图片'}">style="display:none"</c:if>
								...
								<div class="col-sm-10">
									<input type="text" name="title" value="${reply.title }" class="form-control m-b">
								</div>
							</div>
							<div class="form-group"
							<c:if test="${reply.replytype=='文本'}">style="display:none"</c:if> 
								...
								<div class="col-sm-10">
                                	...
                                	<img src="${reply.imgUrl }" name="imgUrl" id="icon" style="height:50px;" maxlength="255" >
								</div>
							</div>
							<div class="form-group"
								<c:if test="${reply.replytype=='图片'}">style="display:none"</c:if>
								...
								<div class="col-sm-10">
									<textarea rows="20" cols="85" name="content" class="form-control m-b">${reply.content}</textarea>
								</div>
							</div>
							<div class="form-group"
								<c:if test="${reply.replytype=='文本'}">style="display:none"</c:if>
								...
								<div class="col-sm-10">
									<input type="text" name="url" value="${reply.url}" size="100" class="form-control m-b">
								</div>
							</div>
							<div class="form-group"
                        		id="tr4">
								<label class="col-sm-2 control-label">
									<font color=red>*</font>是否开启回复：
								</label>
								<div class="col-sm-10">
									<select  class="form-control m-b" id="isOpen" name="isOpen">
										<option value="true" <c:if test="${reply.isOpen=='true'}">selected</c:if>>是</option>
										<option value="false" <c:if test="${reply.isOpen=='false'}">selected</c:if>>否</option>
									</select>
								</div>
							</div>
                            <div>
                            ...
                            <input type="hidden" name="id" value="${reply.id}" size="100">
                        </form>
                    </div>
                </div>
            </div>
