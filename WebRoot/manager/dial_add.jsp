                ...
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                    	<form method="post" class="form-horizontal" >
	                        ...
	                        <div class="form-group " id="jiangpin">
	                        	<!-- 奖品 -->
	                        	<div class="row" id="1">
		                            <label class="col-sm-2 control-label"><font color="red">*</font>奖品1</label>
		                            <div class="col-sm-2">
		                               <select class="form-control" id="level1">
		                            		<option value="0">请选择奖品等级</option>
		                            ...
		                            <div class="col-sm-2">
		                               <select class="form-control "id="type1" onchange="ch('type1')">
		                            		<option value="-1">请选择奖品类型</option>
					                ...
					                <div class="form-group col-sm-3" id="money1" >
		                                <input placeholder="请输入奖品金额" type="text" class="form-control"  size="12"
		                                oninput="document.getElementById('desc1').value=this.value+'元现金';" 
			                        	onpropertychange="document.getElementById('desc1').value=this.value+'元现金';"/>
			                        </div>
			                        <div class="form-group col-sm-3" style="display: none" id="coupon1">
		                            	<select class="form-control" name="coupon" oninput="document.getElementById('desc1').value=this.options[this.selectedIndex].text;" >
			                        ...
			                        <div class="form-group col-sm-3" style="display: none" id="goods1">
		                            	<select class="form-control" name="goods" 
		                            	oninput="document.getElementById('desc1').value=this.options[this.selectedIndex].text;" 
			                        	onpropertychange="document.getElementById('desc1').value=this.options[this.selectedIndex].text;">
		                            		<option value="0">--请选择实物--</option>
		                            		<c:forEach items="${goodsList}" var="each">
		                            			<option value="${each.id }">${each.title }</option>
		                            		</c:forEach>
		                            	</select>
			                        </div>
		                         	<div class="form-group col-sm-3" id="jifen1" style="display:none" >
		                                <input placeholder="请输入积分数量" id="end"  type="text" name="jifen"  class="form-control"  
		                                size="12" oninput="document.getElementById('desc1').value=this.value+'积分';" 
			                        	/>
			                        </div>
			                        <div class="form-group col-sm-2" >
			                        	<input placeholder="请输入奖品名称" type="text" id="desc1" class="form-control" />
		                        	<!-- 减号 -->
	                        			<div class="iconfont icon-jianhao" id="deleteAward1" onclick="del(1)"></div>
			                        </div>
		                        </div>
	                        </div>
	                        
	                        	<!-- 加号 -->
	                       	<div class="iconfont icon-jiahao" style="margin-left:680px;width:50px;" id="addAward"></div>
	                       	<div>
...
<script type="text/javascript">
		...
		$("#addAward").click(function(){
			...
			var se ='<div class="row" id="'+_len+'">'+
	                 	'<label class="col-sm-2 control-label" ><font color="red">*</font>奖品'+_len+'</label>'+
	                     		'<div class="col-sm-2">'+
	                              	'<select class="form-control"  id="level'+_len+'">'+
	                            ...
	                            '</div>'+
	                            '<div class="col-sm-2">'+
	                               '<select class="form-control" id="type'+_len+'" onchange="ch('+"'type"+_len+"'"+')">'+
	                            	...
	                            	'</select>'+
	                           ' </div>'+
				               ' <div class="form-group col-sm-3" id="money'+_len+'">'+
		                         '       <input placeholder="请输入奖品金额" type="text" class="form-control"  size="12"'+
			                         		' oninput=document.getElementById('+"'desc"+_len+"'"+').value=this.value+"元现金";'+ 
				                        	' onpropertychange=document.getElementById('+"'desc"+_len+"'"+').value=this.value+"元现金";/>'+
		                       ' </div>'+
		                       ' <div class="form-group col-sm-3" style="display: none" id="coupon'+_len+'">'+
		                            	'<select class="form-control" name="coupon" '+
			                            	'oninput="document.getElementById('+"'desc"+_len+"'"+').value=this.options[this.selectedIndex].text;" '+
				                        	'onpropertychange="document.getElementById('+"'desc"+_len+"'"+').value=this.options[this.selectedIndex].text;">'+
		                            		'<c:forEach items="${couList}" var="each">'+
		                            		'	<option value="${each.id }">${each.title }</option>'+
		                            		'</c:forEach>'+
		                            	'</select>'+
		                      '  </div>'+
		                       ' <div class="form-group col-sm-3" style="display: none" id="goods'+_len+'">'+
		                            '	<select class="form-control" name="goods"'+
			                            	'oninput="document.getElementById('+"'desc"+_len+"'"+').value=this.options[this.selectedIndex].text;" '+
				                        	'onpropertychange="document.getElementById('+"'desc"+_len+"'"+').value=this.options[this.selectedIndex].text;">'+
		                            	'	<c:forEach items="${goodsList}" var="each">'+
		                            	...
		                            	'</select>'+
		                       ' </div>'+
	                         	'<div class="form-group col-sm-3" id="jifen'+_len+'" style="display:none">'+
		                               ' <input placeholder="请输入积分数量" type="text" class="form-control"  size="12"'+
			                                'oninput="document.getElementById('+"'desc"+_len+"'"+').value=this.value'+"+'积分'"+';" '+
				                        	'onpropertychange="document.getElementById('+"'desc"+_len+"'"+').value=this.value'+"+'积分'"+';"/>'+
		                        '</div>'+
		                        '<div class="form-group col-sm-2" >'+
		                        	'<input placeholder="请输入奖品名称" type="text" class="form-control" id="desc'+_len+'" />'+
		                        	'<div class="iconfont icon-jianhao" id="deleteAward'+_len+'" onclick="del('+_len+')"></div>'+
		                        '</div>'+
	                 '</div>';
			$("#jiangpin").append(se);
			//设置默认值
	...
	$(function(){
		$("#add_activity").click(function(){
			...
			for(var j=1;j<=_len-1;j++){
				...
				//奖品
				var award = {"level":level,"awardtype":awardtype,"type":type,"desc":desc};
				aws.push(award);
			}
			//JSON转字符串
